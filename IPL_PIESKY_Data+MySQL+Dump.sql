...
select * from ipl_match;
select * from ipl_match_schedule;
select s.STADIUM_ID,s.STADIUM_NAME,s.CITY,count(i.MATCH_ID) from ipl_stadium s join ipl_match_schedule i 
on s.STADIUM_ID=i.STADIUM_ID group by STADIUM_NAME ;
...
select * from ipl_bidding_details;
select * from ipl_team;
select BID_TEAM,count(BIDDER_ID),Team_name
from ipl_bidding_details bd join ipl_team t
on bd.BID_TEAM=T.team_id group by bid_team;
...
select * from ipl_team_standings;
select * from ipl_match;
select i.TEAM_ID,i.MATCHES_WON,i.MATCHES_PLAYED,t.REMARKS,t.TOURNMT_ID from ipl_team_standings i 
join ipl_tournament t on i.TOURNMT_ID=t.TOURNMT_ID  ;
...
select * from ipl_team_players;
SELECT * FROM ipl_tournament;
select T.REMARKS,MATCHES_PLAYED,MATCHES_WON,MATCHES_LOST from ipl_team_standings i join ipl_team_players t 
on i.TEAM_ID=t.TEAM_ID JOIN ipl_tournament tn on tn.TOURNMT_ID= i.TOURNMT_ID group by T. REMARKS ;

#7.	Display the bowlers for Mumbai Indians team.
select * from ipl_team_players;
select * from ipl_player;
select * from ipl_team_players where REMARKS like 'TEAM - MI' and PLAYER_ROLE='Bowler';


#8.	How many all-rounders are there in each team, Display the teams with more than 4 
#all-rounder in descending order.
select *  from ipl_team_players;
select count(*) as 'count of allrounder',i1.team_id,i2.team_name from ipl_team_players i1
join ipl_team i2 
on i1.team_id=i2.team_id
join ipl_player i3
on i1.player_id=i3.player_id where i1.player_role='All-Rounder' group by team_id having count(*)>4 order by count(*) desc;
